...
class TwitterOAuth extends Config
{
    ...
    private function uploadMediaChunked($path, array $parameters)
    {
        ...
        while (!feof($media)) {
            $this->http('POST', self::UPLOAD_HOST, 'media/upload', ['command' => 'APPEND', 'media_id' => $init->media_id_string, 'segment_index' => $segmentIndex++, 'media_data' => base64_encode(fread($media, $this->chunkSize))]);
        }
        fclose($media);
        // Finalize
    ...
    {
        $return = ['command' => 'INIT', 'media_type' => $parameters['media_type'], 'total_bytes' => filesize($parameters['media'])];

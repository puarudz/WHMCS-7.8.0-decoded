...
foreach ($frames as $i => $frame) {
  ...
  <div class="frame <?php 
    echo $i == 0 ? 'active' : '';
    ?> <?php 
    echo $frame->isApplication() ? 'frame-application' : '';
    ?>" id="frame-line-<?php 
    echo $i;
    ?>">
    ...
    echo $tpl->breakOnDelimiter('\\', $tpl->escape($frame->getClass() ?: ''));
    ?></span>
        <span class="frame-function"><?php 
    echo $tpl->breakOnDelimiter('\\', $tpl->escape($frame->getFunction() ?: ''));
    ?></span>
      </div>

    <div class="frame-file">
        <?php 
    echo $frame->getFile() ? $tpl->breakOnDelimiter('/', $tpl->shorten($tpl->escape($frame->getFile()))) : '<#unknown>';
    ?><!--

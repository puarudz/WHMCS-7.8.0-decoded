...
class PrettyPageHandler extends Handler
{
     ...
     * @var array[]
     */
    private $blacklist = ['_GET' => [], '_POST' => [], '_FILES' => [], '_COOKIE' => [], '_SESSION' => [], '_SERVER' => [], '_ENV' => []];
    /**
    ...
    public function handle()
    {
        ...
        $vars = [
            ...
            "tables" => ["GET Data" => $this->masked($_GET, '_GET'), "POST Data" => $this->masked($_POST, '_POST'), "Files" => isset($_FILES) ? $this->masked($_FILES, '_FILES') : [], "Cookies" => $this->masked($_COOKIE, '_COOKIE'), "Session" => isset($_SESSION) ? $this->masked($_SESSION, '_SESSION') : [], "Server/Request Data" => $this->masked($_SERVER, '_SERVER'), "Environment Variables" => $this->masked($_ENV, '_ENV')],
        ];
    ...
    protected function getEditor($filePath, $line)
    {
        ...
        if (is_string($this->editor) && isset($this->editors[$this->editor]) && !is_callable($this->editors[$this->editor])) {
            return ['ajax' => false, 'url' => $this->editors[$this->editor]];
        }
        if (is_callable($this->editor) || isset($this->editors[$this->editor]) && is_callable($this->editors[$this->editor])) {
            ...
            if (is_string($callback)) {
                return ['ajax' => false, 'url' => $callback];
            }
            return ['ajax' => isset($callback['ajax']) ? $callback['ajax'] : false, 'url' => isset($callback['url']) ? $callback['url'] : $callback];
        }
        return [];
    }
    /**

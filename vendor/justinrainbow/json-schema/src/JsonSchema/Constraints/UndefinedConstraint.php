...
class UndefinedConstraint extends Constraint
{
    ...
    public function check($value, $schema = null, $path = null, $i = null)
    {
        ...
        if (!is_object($schema)) {
            throw new InvalidArgumentException('Given schema must be an object in ' . $path . ' but is a ' . gettype($schema));
        }
        $i = is_null($i) ? "" : $i;
    ...
    public function validateTypes($value, $schema = null, $path = null, $i = null)
    {
        ...
        if (is_object($value) && (isset($schema->properties) || isset($schema->patternProperties) || isset($schema->additionalProperties))) {
            $this->checkObject($value, isset($schema->properties) ? $schema->properties : null, $path, isset($schema->additionalProperties) ? $schema->additionalProperties : null, isset($schema->patternProperties) ? $schema->patternProperties : null);
        }
        // check string
        if (is_string($value)) {
    ...
    protected function validateOfProperties($value, $schema, $path, $i = "")
    {
        ...
        if (isset($schema->oneOf)) {
            ...
            if ($matchedSchemas !== 1) {
                $this->addErrors(array_merge($allErrors, array(array('property' => $path, 'message' => "Failed to match exactly one schema", 'constraint' => 'oneOf')), $startErrors));
            } else {
                $this->errors = $startErrors;
            }
        }

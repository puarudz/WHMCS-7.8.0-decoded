...
try {
    ...
    $output = array();
    exec('php' . ' ' . escapeshellarg(ROOT_DIR . DIRECTORY_SEPARATOR . 'vendor' . DIRECTORY_SEPARATOR . 'apigen' . DIRECTORY_SEPARATOR . 'apigen' . DIRECTORY_SEPARATOR . 'src' . DIRECTORY_SEPARATOR . 'apigen.php') . ' generate' . ' --source=' . escapeshellarg(SOURCE_DIR) . ' --destination=' . escapeshellarg(DEST_DIR) . ' --access-levels=public' . ' --extensions=php' . ' --groups=namespaces' . ' --charset=UTF-8' . ' --template-theme=bootstrap' . ' --title=' . escapeshellarg('Punic APIs') . ' --todo' . ' --tree' . ' --debug' . ' 2>&1', $output, $rc);
    if ($rc !== 0) {

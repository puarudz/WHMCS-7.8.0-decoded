...
function buildCLDRJson()
{
    ...
    try {
        ...
        foreach ($locales as $locale) {
            ...
            $cmd .= ' -t main';
            // (main|supplemental|segments) Type of CLDR data being generated, main, supplemental, or segments.
            $cmd .= ' -r true';
            // (true|false) Whether the output JSON for the main directory should be based on resolved or unresolved data
            $cmd .= ' -m ' . escapeshellarg(str_replace('-', '_', $locale));
...
function copyData()
{
        ...
        if (is_dir($src . DIRECTORY_SEPARATOR . $locale)) {
            ...
            foreach ($copy as $copyFrom => $info) {
                if ($info['kind'] === 'main') {
                    ...
                    $info['unsetByPath'] = array_merge(isset($info['unsetByPath']) ? $info['unsetByPath'] : array(), array("/main/{$useLocale}" => array('identity')));
    ...
    foreach ($copy as $copyFrom => $info) {
        if ($info['kind'] === 'supplemental') {
            ...
            $info['unsetByPath'] = array_merge(isset($info['unsetByPath']) ? $info['unsetByPath'] : array(), array('/supplemental' => array('version', 'generation')));
            copyDataFile($srcFile, $info, $dstFile);
        }
    }
    echo "done.\n";
                ...
                foreach ($keys as $key) {
                    ...
                    if ($rule === 'other') {
                        ...
                        foreach (array('^', ' and ', ' or ') as $pre) {
                            while (preg_match('/' . $pre . '(([nivfwft]( % \\d+)?) (==|!=) ((\\d+)(((\\.\\.)|,)+(\\d+))+))/', $v, $m)) {
                                $found = $m[1];
                                $leftyPart = $m[2];
                                // eg 'n % 10'
                                $operator = $m[4];
...
function numberFormatToRegularExpressions($symbols, $isoPattern)
{
    ...
    $patterns = array('+' => $p[0], '-' => count($p) == 1 ? "-{$p[0]}" : $p[1]);
    $result = array();
    $m = null;

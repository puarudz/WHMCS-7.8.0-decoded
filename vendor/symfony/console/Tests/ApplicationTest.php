...
class ApplicationTest extends \PHPUnit_Framework_TestCase
{
    ...
    {
        return array(array('f', 'Command "f" is not defined.'), array('a', 'Command "a" is ambiguous (afoobar, afoobar1 and 1 more).'), array('foo:b', 'Command "foo:b" is ambiguous (foo:bar, foo:bar1 and 1 more).'));
    }
    ...
    {
        return array(array('foo3:baR'), array('foO3:bar'));
    }
    ...
    public function testFindNamespaceDoesNotFailOnDeepSimilarNamespaces()
    {
        ...
        $application->expects($this->once())->method('getNamespaces')->will($this->returnValue(array('foo:sublong', 'bar:sub')));
    ...
    public function testRenderException()
    {
        ...
        $application = $this->getMock('Symfony\\Component\\Console\\Application', array('getTerminalWidth'));
        $application->setAutoExit(false);
        $application->expects($this->any())->method('getTerminalWidth')->will($this->returnValue(32));
        $tester = new ApplicationTester($application);
        $tester->run(array('command' => 'foo'), array('decorated' => false));
    ...
    public function testRunReturnsIntegerExitCode()
    {
        ...
        $application->setAutoExit(false);
        $application->expects($this->once())->method('doRun')->will($this->throwException($exception));
    ...
    public function testAddingOptionWithDuplicateShortcut()
    {
        ...
        $application->register('foo')->setAliases(array('f'))->setDefinition(array(new InputOption('survey', 'e', InputOption::VALUE_REQUIRED, 'My option with a shortcut.')))->setCode(function (InputInterface $input, OutputInterface $output) {
        });
    ...
    {
        return array(array(new InputArgument('command', InputArgument::REQUIRED)), array(new InputOption('quiet', '', InputOption::VALUE_NONE)), array(new InputOption('query', 'q', InputOption::VALUE_NONE)));
    }

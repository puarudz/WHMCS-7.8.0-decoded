...
class NormalizationTest extends TestCase
{
    ...
    public function testNormalizeEncoders($denormalized)
    {
        ...
        $normalized = array('encoders' => array('foo' => array('algorithm' => 'plaintext')));
    ...
    public function getEncoderTests()
    {
        $configs = array();
        // XML
        $configs[] = array('encoder' => array(array('class' => 'foo', 'algorithm' => 'plaintext')));
        // XML when only one element of this type
        $configs[] = array('encoder' => array('class' => 'foo', 'algorithm' => 'plaintext'));
        // YAML/PHP
        $configs[] = array('encoders' => array(array('class' => 'foo', 'algorithm' => 'plaintext')));
        // YAML/PHP
        $configs[] = array('encoders' => array('foo' => 'plaintext'));
        // YAML/PHP
        $configs[] = array('encoders' => array('foo' => array('algorithm' => 'plaintext')));
    ...
    {
        $configs = array();
        $configs[] = array('logout' => array('handlers' => array('a', 'b', 'c')));
        $configs[] = array('logout' => array('handler' => array('a', 'b', 'c')));
        return array_map(function ($v) {
            return array($v);
        }, $configs);
    }
    ...
    {
        $normalized = array('thing' => array(42 => array('foo', 'bar'), 1337 => array('baz', 'qux')));
        $this->assertNormalized($this->getNumericKeysTestTree(), $denormalized, $normalized);
    }
    public function getNumericKeysTests()
    {
        $configs = array();
        $configs[] = array('thing' => array(42 => array('foo', 'bar'), 1337 => array('baz', 'qux')));
        $configs[] = array('thing' => array(array('foo', 'bar', 'id' => 42), array('baz', 'qux', 'id' => 1337)));
        return array_map(function ($v) {
            return array($v);
        }, $configs);
    }
    ...
    {
        $denormalized = array('thing' => array(array('foo', 'bar'), array('baz', 'qux')));

...
class ParameterBagTest extends \PHPUnit_Framework_TestCase
{
    ...
    {
        $bag = new ParameterBag($parameters = array('foo' => 'foo', 'bar' => 'bar'));
    ...
    {
        $bag = new ParameterBag(array('foo' => 'foo', 'bar' => 'bar', 'baz' => 'baz'));
        try {
            $bag->get('foo1');
            $this->fail('->get() throws an Symfony\\Component\\DependencyInjection\\Exception\\ParameterNotFoundException if the key does not exist');
        } catch (\Exception $e) {
    ...
    {
        return array(array('bar', '%foo%', 'Parameters must be wrapped by %.'), array('% foo %', '% foo %', 'Parameters should not have spaces.'), array('{% set my_template = "foo" %}', '{% set my_template = "foo" %}', 'Twig-like strings are not parameters.'), array('50% is less than 100%', '50% is less than 100%', 'Text between % signs is allowed, if there are spaces.'));
    }
}

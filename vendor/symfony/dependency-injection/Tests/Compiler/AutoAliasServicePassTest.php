...
class AutoAliasServicePassTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function testProcessWithMissingParameter()
    {
        ...
        $container->register('example')->addTag('auto_alias', array('format' => '%non_existing%.example'));
    ...
    {
        $container = new ContainerBuilder();
        $container->register('example')->addTag('auto_alias', array());
    ...
    {
        $container = new ContainerBuilder();
        $container->register('example', 'Symfony\\Component\\DependencyInjection\\Tests\\Compiler\\ServiceClassDefault')->addTag('auto_alias', array('format' => '%existing%.example'));
        $container->setParameter('existing', 'mysql');
        $pass = new AutoAliasServicePass();
        $pass->process($container);

...
class AutowirePassTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function testSomeSpecificArgumentsAreSet()
    {
        ...
        $container->register('multiple', __NAMESPACE__ . '\\MultipleArguments')->setAutowired(true)->setArguments(array(1 => new Reference('foo')));
        $pass = new AutowirePass();
        $pass->process($container);
        $definition = $container->getDefinition('multiple');
        $this->assertEquals(array(new Reference('a'), new Reference('foo'), new Reference('dunglas')), $definition->getArguments());
    }
    /**
     * @expectedException \Symfony\Component\DependencyInjection\Exception\RuntimeException
    ...
    public function testScalarArgsCannotBeAutowired()
    {
        ...
        $container->register('a', __NAMESPACE__ . '\\A');
        $container->register('dunglas', __NAMESPACE__ . '\\Dunglas');
        $container->register('arg_no_type_hint', __NAMESPACE__ . '\\MultipleArguments')->setAutowired(true);
        $pass = new AutowirePass();
        $pass->process($container);
    ...
    public function testOptionalScalarNotReallyOptionalThrowException()
    {
        ...
        $container->register('not_really_optional_scalar', __NAMESPACE__ . '\\MultipleArgumentsOptionalScalarNotReallyOptional')->setAutowired(true);
        $pass = new AutowirePass();
        $pass->process($container);
    }
    public function testOptionalScalarArgsDontMessUpOrder()
    {
        ...
        $container->register('a', __NAMESPACE__ . '\\A');
        $container->register('lille', __NAMESPACE__ . '\\Lille');
        $container->register('with_optional_scalar', __NAMESPACE__ . '\\MultipleArgumentsOptionalScalar')->setAutowired(true);
        $pass = new AutowirePass();
        $pass->process($container);
    ...
    {
        return array(array('IdenticalClassResource', true), array('ClassChangedConstructorArgs', false));
    }
    ...
    public function testEmptyStringIsKept()
    {
        ...
        $container->register('a', __NAMESPACE__ . '\\A');
        $container->register('lille', __NAMESPACE__ . '\\Lille');
        $container->register('foo', __NAMESPACE__ . '\\MultipleArgumentsOptionalScalar')->setAutowired(true)->setArguments(array('', ''));
        $pass = new AutowirePass();
        $pass->process($container);

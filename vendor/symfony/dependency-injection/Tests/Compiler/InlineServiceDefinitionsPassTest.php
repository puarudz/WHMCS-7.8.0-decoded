...
class InlineServiceDefinitionsPassTest extends \PHPUnit_Framework_TestCase
{
    public function testProcess()
    {
        ...
        $container->register('service')->setArguments(array(new Reference('inlinable.service')));
    ...
    public function testProcessDoesNotInlinesWhenAliasedServiceIsShared()
    {
        ...
        $container->register('service')->setArguments(array($ref = new Reference('foo')));
        $this->process($container);
        $arguments = $container->getDefinition('service')->getArguments();
    ...
    public function testProcessDoesInlineNonSharedService()
    {
        ...
        $container->register('service')->setArguments(array(new Reference('foo'), $ref = new Reference('moo'), new Reference('bar')));
        $this->process($container);
        $arguments = $container->getDefinition('service')->getArguments();
    ...
    public function testProcessInlinesIfMultipleReferencesButAllFromTheSameDefinition()
    {
        ...
        $b = $container->register('b')->addArgument(new Reference('a'))->addArgument(new Definition(null, array(new Reference('a'))));
        $this->process($container);
    ...
    public function testProcessInlinesPrivateFactoryReference()
    {
        ...
        $b = $container->register('b')->setPublic(false)->setFactory(array(new Reference('a'), 'a'));
        $container->register('foo')->setArguments(array($ref = new Reference('b')));
        $this->process($container);
    ...
    public function testProcessDoesNotInlinePrivateFactoryIfReferencedMultipleTimesWithinTheSameDefinition()
    {
        ...
        $container->register('b')->setPublic(false)->setFactory(array(new Reference('a'), 'a'));
        $container->register('foo')->setArguments(array($ref1 = new Reference('b'), $ref2 = new Reference('b')));
        $this->process($container);
    ...
    public function testProcessDoesNotInlineReferenceWhenUsedByInlineFactory()
    {
        ...
        $container->register('foo')->setArguments(array($ref = new Reference('b'), $inlineFactory));
        $this->process($container);
        $args = $container->getDefinition('foo')->getArguments();
    ...
    {
        $container = new ContainerBuilder();
        $container->register('foo')->setPublic(false)->addMethodCall('foo', array($ref = new Reference('foo')));
        $this->process($container);

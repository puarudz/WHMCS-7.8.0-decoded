...
$LEXERS = array();
$RUNS = isset($GLOBALS['HTMLPurifierTest']['Runs']) ? $GLOBALS['HTMLPurifierTest']['Runs'] : 2;
...
class RowTimer extends Benchmark_Timer
{
    ...
    public function getOutput()
    {
        ...
        foreach ($result as $k => $v) {
            ...
            $bad_run = $v['diff'] < 0;
            $out .= '<td align="right"' . ($bad_run ? ' style="color:#AAA;"' : '') . '>' . number_format($perc, 2, '.', '') . '%</td><td>' . number_format($v['diff'], 4, '.', '') . '</td>';
        }
        $out .= '</tr>';
        return $out;
    }

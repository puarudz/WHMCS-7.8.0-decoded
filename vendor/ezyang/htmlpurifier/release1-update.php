...
if ($is_dev = strpos($version, 'dev') === false) {
    $date = date('Y-m-d');
    $news_c = str_replace($l = "{$version}, unknown release date", "{$version}, released {$date}", file_get_contents('NEWS'), $c);
    if (!$c) {
...
$htmlpurifier_c = preg_replace('/HTML Purifier .+? - /', "HTML Purifier {$version} - ", $htmlpurifier_c, 1, $c);
if (!$c) {
    echo 'Could not update HTMLPurifier.php, missing HTML Purifier [version] header.' . PHP_EOL;
    exit;
}
$htmlpurifier_c = preg_replace('/public \\$version = \'.+?\';/', "public \$version = '{$version}';", $htmlpurifier_c, 1, $c);
if (!$c) {
    echo 'Could not update HTMLPurifier.php, missing public $version.' . PHP_EOL;
    exit;
}
$htmlpurifier_c = preg_replace('/const VERSION = \'.+?\';/', "const VERSION = '{$version}';", $htmlpurifier_c, 1, $c);
if (!$c) {
...
$config_c = preg_replace('/public \\$version = \'.+?\';/', "public \$version = '{$version}';", $config_c, 1, $c);
if (!$c) {

...
{
    return str_replace(array("\t", '»', '\\0(null)'), array('<strong>\\t</strong>', '<span class="linebreak">»</span>', '<strong>\\0</strong>'), escapeHTML(str_replace("\0", '\\0(null)', wordwrap($string, 28, " »\n", true))));
}
?><!DOCTYPE html
...
foreach ($xml->attack as $attack) {
    ...
    ?>>
        <td><?php 
    echo escapeHTML($attack->name);
    ?></td>
        <td><pre><?php 
    echo formatCode($code);
    ?></pre></td>
        <?php 
    $pure_html = $purifier->purify($code);
    ?>
        <td><pre><?php 
    echo formatCode($pure_html);
    ?></pre></td>

...
class AccessToken implements AccessTokenInterface
{
    ...
    public function __construct(AccessTokenStorageInterface $tokenStorage, RefreshTokenInterface $refreshStorage = null, array $config = array())
    {
        ...
        $this->config = array_merge(array('token_type' => 'bearer', 'access_lifetime' => 3600, 'refresh_token_lifetime' => 1209600), $config);
    }
    ...
    {
        $token = array("access_token" => $this->generateAccessToken(), "expires_in" => $this->config['access_lifetime'], "token_type" => $this->config['token_type'], "scope" => $scope);
    ...
    public function revokeToken($token, $tokenTypeHint = null)
    {
        ...
        if (!method_exists($this->tokenStorage, 'unsetAccessToken')) {
            throw new \RuntimeException(sprintf('Token storage %s must implement unsetAccessToken method', get_class($this->tokenStorage)));
        }

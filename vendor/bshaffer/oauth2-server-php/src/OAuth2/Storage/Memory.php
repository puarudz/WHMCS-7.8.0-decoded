...
class Memory implements AuthorizationCodeInterface, UserCredentialsInterface, UserClaimsInterface, AccessTokenInterface, ClientCredentialsInterface, RefreshTokenInterface, JwtBearerInterface, ScopeInterface, PublicKeyInterface, OpenIDAuthorizationCodeInterface
{
    ...
    {
        $params = array_merge(array('authorization_codes' => array(), 'user_credentials' => array(), 'client_credentials' => array(), 'refresh_tokens' => array(), 'access_tokens' => array(), 'jwt' => array(), 'jti' => array(), 'default_scope' => null, 'supported_scopes' => array(), 'keys' => array()), $params);
    ...
    public function getAuthorizationCode($code)
    {
        ...
        return array_merge(array('authorization_code' => $code), $this->authorizationCodes[$code]);
    }
    ...
    {
        $this->userCredentials[$username] = array('password' => $password, 'first_name' => $firstName, 'last_name' => $lastName);
        return true;
    }
    public function getUserDetails($username)
    {
        ...
        return array_merge(array('user_id' => $username, 'password' => null, 'first_name' => null, 'last_name' => null), $this->userCredentials[$username]);
    }
    /* UserClaimsInterface */
    ...
    public function getClientDetails($client_id)
    {
        ...
        $clientDetails = array_merge(array('client_id' => $client_id, 'client_secret' => null, 'redirect_uri' => null, 'scope' => null), $this->clientCredentials[$client_id]);
    ...
    {
        $this->clientCredentials[$client_id] = array('client_id' => $client_id, 'client_secret' => $client_secret, 'redirect_uri' => $redirect_uri, 'grant_types' => $grant_types, 'scope' => $scope, 'user_id' => $user_id);
        return true;
    }
    ...
    public function getJti($client_id, $subject, $audience, $expires, $jti)
    {
        foreach ($this->jti as $storedJti) {
            if ($storedJti['issuer'] == $client_id && $storedJti['subject'] == $subject && $storedJti['audience'] == $audience && $storedJti['expires'] == $expires && $storedJti['jti'] == $jti) {
                return array('issuer' => $storedJti['issuer'], 'subject' => $storedJti['subject'], 'audience' => $storedJti['audience'], 'expires' => $storedJti['expires'], 'jti' => $storedJti['jti']);
            }
        }
        return null;
    }

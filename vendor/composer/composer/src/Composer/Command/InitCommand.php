...
EOT
    ...
    protected function interact(InputInterface $input, OutputInterface $output)
    {
        ...
        $type = $io->ask('Package Type [<comment>' . $type . '</comment>]: ', $type);
    ...
    public function parseAuthorString($author)
    {
        if (preg_match('/^(?P<name>[- \\.,\\p{L}\\p{N}\'’]+) <(?P<email>.+?)>$/u', $author, $match)) {
            if ($this->isValidEmail($match['email'])) {
                return array('name' => trim($match['name']), 'email' => $match['email']);
            }
        }
    ...
    {
        if (!$this->repos) {
            $this->repos = new CompositeRepository(array_merge(array(new PlatformRepository()), RepositoryFactory::defaultRepos($this->getIO())));
        }
        return $this->repos;
    }
    ...
    private function findBestVersionForPackage(InputInterface $input, $name, $phpVersion)
    {
        ...
        if (!$package) {
            ...
            if ($phpVersion && $versionSelector->findBestCandidate($name)) {
                throw new \InvalidArgumentException(sprintf('Could not find package %s at any version matching your PHP version %s', $name, $phpVersion));
            }

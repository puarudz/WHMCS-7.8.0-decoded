...
class JsonManipulator
{
    ...
    public function addLink($type, $package, $constraint, $sortPackages = false)
    {
        ...
        $regex = '{^(\\s*\\{\\s*(?:' . self::$JSON_STRING . '\\s*:\\s*' . self::$JSON_VALUE . '\\s*,\\s*)*?)' . '(' . preg_quote(JsonFile::encode($type)) . '\\s*:\\s*)(' . self::$JSON_VALUE . ')(.*)}s';
    ...
    public function addSubNode($mainNode, $name, $value)
    {
        ...
        $nodeRegex = '{^(\\s*\\{\\s*(?:' . self::$JSON_STRING . '\\s*:\\s*' . self::$JSON_VALUE . '\\s*,\\s*)*?)' . '(' . preg_quote(JsonFile::encode($mainNode)) . '\\s*:\\s*\\{)(' . self::$RECURSE_BLOCKS . ')(\\})(.*)}s';
        try {
        ...
        if ($this->pregMatch('{("' . preg_quote($name) . '"\\s*:\\s*)(' . self::$JSON_VALUE . ')(,?)}', $children, $matches)) {
            ...
            $children = preg_replace('#' . $match[1] . '$#', addcslashes(',' . $this->newline . $this->indent . $this->indent . JsonFile::encode($name) . ': ' . $this->format($value, 1) . $match[1], '\\$'), $children);
        } else {
            if ($subName !== null) {
                $value = array($subName => $value);
            }

...
class AuthHelper
{
    ...
    public function storeAuth($originUrl, $storeAuth)
    {
        ...
        if ($storeAuth === true) {
            ...
            $answer = $this->io->askAndValidate('Do you want to store credentials for ' . $originUrl . ' in ' . $configSource->getName() . ' ? [Yn] ', function ($value) {
        ...
        if ($store) {
            $store->addConfigSetting('http-basic.' . $originUrl, $this->io->getAuthentication($originUrl));
        }
    }
}

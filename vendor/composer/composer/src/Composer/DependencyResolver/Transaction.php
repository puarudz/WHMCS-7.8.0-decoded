        ...
        foreach ($this->decisions as $i => $decision) {
            ...
            if ($literal > 0) {
                if (isset($installMeansUpdateMap[abs($literal)]) && !$package instanceof AliasPackage) {
                    ...
                    $updateMap[$package->id] = array('package' => $package, 'source' => $source, 'reason' => $reason);
                    // avoid updates to one package from multiple origins
                    unset($installMeansUpdateMap[abs($literal)]);
                    $ignoreRemove[$source->id] = true;
                } else {
                    $installMap[$package->id] = array('package' => $package, 'reason' => $reason);
                }
            }
        }
        foreach ($this->decisions as $i => $decision) {
            ...
            $reason = $decision[Decisions::DECISION_REASON];
            $package = $this->pool->literalToPackage($literal);
            if ($literal <= 0 && isset($this->installedMap[$package->id]) && !isset($ignoreRemove[$package->id])) {
                $uninstallMap[$package->id] = array('package' => $package, 'reason' => $reason);
            }
        }
        ...
        while (!empty($queue)) {
            ...
            if (!isset($visited[$packageId])) {
                ...
                if (isset($installMap[$packageId])) {
                    $this->install($installMap[$packageId]['package'], $installMap[$packageId]['reason']);
                    unset($installMap[$packageId]);
                }
                if (isset($updateMap[$packageId])) {
                    $this->update($updateMap[$packageId]['source'], $updateMap[$packageId]['package'], $updateMap[$packageId]['reason']);

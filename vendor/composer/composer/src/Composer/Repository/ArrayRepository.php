...
class ArrayRepository extends BaseRepository
{
    ...
    public function search($query, $mode = 0)
    {
        ...
        foreach ($this->getPackages() as $package) {
            ...
            if (preg_match($regex, $name) || $mode === self::SEARCH_FULLTEXT && $package instanceof CompletePackageInterface && preg_match($regex, implode(' ', (array) $package->getKeywords()) . ' ' . $package->getDescription())) {
                $matches[$name] = array('name' => $package->getPrettyName(), 'description' => $package->getDescription());
            }
        }

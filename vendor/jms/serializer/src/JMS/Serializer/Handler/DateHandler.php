...
class DateHandler implements SubscribingHandlerInterface
{
    ...
    public static function getSubscribingMethods()
    {
        ...
        foreach (array('json', 'xml', 'yml') as $format) {
            foreach ($deserializationTypes as $type) {
                $methods[] = ['type' => $type, 'direction' => GraphNavigator::DIRECTION_DESERIALIZATION, 'format' => $format];
            }
            foreach ($serialisationTypes as $type) {
                $methods[] = array('type' => $type, 'format' => $format, 'direction' => GraphNavigator::DIRECTION_SERIALIZATION, 'method' => 'serialize' . $type);
            }
        }
        return $methods;
    }
    ...
    private function serializeDateTimeInterface(VisitorInterface $visitor, \DateTimeInterface $date, array $type, Context $context)
    {
    ...
    public function serializeDateTimeImmutable(VisitorInterface $visitor, \DateTimeImmutable $date, array $type, Context $context)
    {
        return $this->serializeDateTimeInterface($visitor, $date, $type, $context);
    }

...
final class GraphNavigator
{
    ...
    public function __construct(MetadataFactoryInterface $metadataFactory, HandlerRegistryInterface $handlerRegistry, ObjectConstructorInterface $objectConstructor, EventDispatcherInterface $dispatcher = null, ExpressionEvaluatorInterface $expressionEvaluator = null)
    {
    ...
    private function resolveMetadata($data, ClassMetadata $metadata)
    {
        switch (true) {
            ...
            case is_object($data) && isset($data->{$metadata->discriminatorFieldName}):
                ...
                break;
            default:
                throw new \LogicException(sprintf('The discriminator field name "%s" for base-class "%s" was not found in input data.', $metadata->discriminatorFieldName, $metadata->name));
        }

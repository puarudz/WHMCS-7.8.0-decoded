...
class Smarty_Internal_Compile_Section extends Smarty_Internal_Compile_Private_ForeachSection
{
    ...
    public function compile($args, Smarty_Internal_TemplateCompilerBase $compiler)
    {
        ...
        if (!isset($propValue['start'])) {
            ...
            $start_code = array(1 => "{$propValue['start']} < 0 ? ", 2 => 'max(', 3 => "{$propValue['step']} > 0 ? ", 4 => '0', 5 => ' : ', 6 => '-1', 7 => ', ', 8 => "{$propValue['start']} + {$propValue['loop']}", 10 => ')', 11 => ' : ', 12 => 'min(', 13 => $propValue['start'], 14 => ', ', 15 => "{$propValue['step']} > 0 ? ", 16 => $propValue['loop'], 17 => ' : ', 18 => $propType['loop'] === 0 ? $propValue['loop'] - 1 : "{$propValue['loop']} - 1", 19 => ')');
            if ($propType['step'] === 0) {
            ...
            if ($propType['start'] === 0) {
                ...
                if ($propValue['start'] < 0) {
                    ...
                    if ($propType['start'] === 0) {
                        $start_code = array(max($propValue['step'] > 0 ? 0 : -1, $propValue['start'] + $propValue['loop']));
                    }
                } else {
                    for ($i = 1; $i <= 11; $i++) {
                        $start_code[$i] = '';
                    ...
                    if ($propType['start'] === 0) {
                        $start_code = array(min($propValue['step'] > 0 ? $propValue['loop'] : $propValue['loop'] - 1, $propValue['start']));
                    }
                }
            }
            $propValue['start'] = join('', $start_code);
        ...
        if (!isset($_attr['start']) && !isset($_attr['step']) && !isset($_attr['max'])) {
            ...
            if ($propType['total'] === 0) {
                ...
                $total_code = array(1 => 'min(', 2 => 'ceil(', 3 => '(', 4 => "{$propValue['step']} > 0 ? ", 5 => $propValue['loop'], 6 => ' - ', 7 => $propValue['start'], 8 => ' : ', 9 => $propValue['start'], 10 => '+ 1', 11 => ')', 12 => '/ ', 13 => 'abs(', 14 => $propValue['step'], 15 => ')', 16 => ')', 17 => ", {$propValue['max']})");
                if (!isset($propValue['max'])) {

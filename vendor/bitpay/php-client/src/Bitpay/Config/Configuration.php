...
class Configuration implements ConfigurationInterface
{
    ...
    public function getConfigTreeBuilder()
    {
        ...
        $rootNode->children()->scalarNode('public_key')->info('Public Key Filename')->defaultValue($this->getPubKeyFilename())->end()->scalarNode('private_key')->info('Private Key Filename')->defaultValue($this->getPriKeyFilename())->end()->scalarNode('sin_key')->info('SIN Filename')->defaultValue($this->getSinFilename())->end()->enumNode('network')->values($this->networks)->info('Network')->defaultValue($this->networks[0])->end()->enumNode('adapter')->values($this->adapters)->info('Client Adapter')->defaultValue($this->adapters[0])->end()->append($this->addKeyStorageNode())->scalarNode('key_storage_password')->info('Used to encrypt and decrypt keys when saving to filesystem')->defaultNull()->end()->end();
        return $treeBuilder;
    }
    /**
    ...
    protected function addKeyStorageNode()
    {
        ...
        $node->info('Class that is used to store your keys')->defaultValue($this->defstorage)->validate()->always()->then(function ($value) {

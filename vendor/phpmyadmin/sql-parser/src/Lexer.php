...
class Lexer extends Core
{
    ...
    public function lex()
    {
        ...
        for ($this->last = 0, $lastIdx = 0; $this->last < $this->len; $lastIdx = ++$this->last) {
            ...
            if ($token === null) {
            ...
            } elseif ($lastToken !== null && $token->type === Token::TYPE_SYMBOL && $token->flags & Token::FLAG_SYMBOL_VARIABLE && ($lastToken->type === Token::TYPE_STRING || $lastToken->type === Token::TYPE_SYMBOL && $lastToken->flags & Token::FLAG_SYMBOL_BACKTICK)) {
                // Handles ```... FROM 'user'@'%' ...```.
                ...
                continue;
            } elseif ($lastToken !== null && $token->type === Token::TYPE_KEYWORD && $lastToken->type === Token::TYPE_OPERATOR && $lastToken->value === '.') {
    ...
    public function parseString($quote = '')
    {
        ...
        if ($this->last >= $this->len || $this->str[$this->last] !== $quote) {
            $this->error(sprintf(Translator::gettext('Ending quote %1$s was expected.'), $quote), '', $this->last);
        } else {
            $token .= $this->str[$this->last];
        }

...
class CaseExpression extends Component
{
    ...
    public static function parse(Parser $parser, TokensList $list, array $options = array())
    {
        ...
        for (; $list->idx < $list->count; ++$list->idx) {
            ...
            if ($state === 0) {
                if ($token->type === Token::TYPE_KEYWORD && $token->keyword === 'WHEN') {
                    ++$list->idx;
                    // Skip 'WHEN'
                ...
                } elseif ($token->type === Token::TYPE_KEYWORD && $token->keyword === 'ELSE') {
                    ++$list->idx;
                    // Skip 'ELSE'
                    $ret->else_result = Expression::parse($parser, $list);
                    $state = 0;
                    // last clause of CASE expression
                } elseif ($token->type === Token::TYPE_KEYWORD && $token->keyword === 'END') {
                    $state = 3;

...
class DeleteStatement extends Statement
{
    ...
    public function parse(Parser $parser, TokensList $list)
    {
        ...
        for (; $list->idx < $list->count; ++$list->idx) {
            ...
            if ($state === 0) {
                if ($token->type === Token::TYPE_KEYWORD && $token->keyword !== 'FROM') {
                    $parser->error('Unexpected keyword.', $token);
                    break;
                } elseif ($token->type === Token::TYPE_KEYWORD && $token->keyword === 'FROM') {
                    ++$list->idx;
                    // Skip 'FROM'
                ...
                if ($token->type === Token::TYPE_KEYWORD && stripos($token->keyword, 'JOIN') !== false) {
                    ++$list->idx;
                    $this->join = JoinKeyword::parse($parser, $list);
                    // remain in state = 2
                } elseif ($token->type === Token::TYPE_KEYWORD && $token->keyword === 'USING') {
                    ++$list->idx;
                    // Skip 'USING'
                    $this->using = ExpressionArray::parse($parser, $list);
                    $state = 3;
                    $multiTable = true;
                } elseif ($token->type === Token::TYPE_KEYWORD && $token->keyword === 'WHERE') {
                    ++$list->idx;
                    // Skip 'WHERE'
                    $this->where = Condition::parse($parser, $list);
                    $state = 4;
                } elseif ($token->type === Token::TYPE_KEYWORD && $token->keyword === 'ORDER BY') {
                    ++$list->idx;
                    // Skip 'ORDER BY'
                    $this->order = OrderKeyword::parse($parser, $list);
                    $state = 5;
                } elseif ($token->type === Token::TYPE_KEYWORD && $token->keyword === 'LIMIT') {
                    ++$list->idx;
                    // Skip 'LIMIT'
                ...
                if ($token->type === Token::TYPE_KEYWORD && $token->keyword === 'WHERE') {
                    ++$list->idx;
                    // Skip 'WHERE'
                    $this->where = Condition::parse($parser, $list);
                    $state = 4;
                ...
                if ($token->type === Token::TYPE_KEYWORD && $token->keyword === 'ORDER BY') {
                    ++$list->idx;
                    // Skip 'ORDER  BY'
                    $this->order = OrderKeyword::parse($parser, $list);
                    $state = 5;
                ...
                if ($token->type === Token::TYPE_KEYWORD && $token->keyword === 'LIMIT') {
                    ++$list->idx;
                    // Skip 'LIMIT'
                    $this->limit = Limit::parse($parser, $list);
                    $state = 6;

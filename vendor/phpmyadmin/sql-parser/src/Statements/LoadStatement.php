...
class LoadStatement extends Statement
{
    ...
    {
        $ret = 'LOAD DATA ' . $this->options . ' INFILE ' . $this->file_name;
    ...
    public function parse(Parser $parser, TokensList $list)
    {
        ...
        for (; $list->idx < $list->count; ++$list->idx) {
            ...
            if ($state === 0) {
                ...
                ++$list->idx;
                $this->file_name = Expression::parse($parser, $list, array('parseField' => 'file'));
                $state = 1;
            } elseif ($state === 1) {
                if ($token->type === Token::TYPE_KEYWORD && ($token->keyword === 'REPLACE' || $token->keyword === 'IGNORE')) {
                    $this->replace_ignore = trim($token->keyword);
                } elseif ($token->type === Token::TYPE_KEYWORD && $token->keyword === 'INTO') {
                    $state = 2;
                }
            } elseif ($state === 2) {
                ...
                if ($token->type === Token::TYPE_KEYWORD) {
                    ...
                    $this->col_name_or_user_var = ExpressionArray::parse($parser, $list);
                    $state = 7;
                } else {
                    $parser->error('Unexpected token.', $token);
                    break;
        ...
        switch ($state) {
            ...
            case 6:
                if ($token->keyword === 'IGNORE') {
                    ...
                    if ($nextToken->type === Token::TYPE_KEYWORD && ($nextToken->keyword === 'LINES' || $nextToken->keyword === 'ROWS')) {

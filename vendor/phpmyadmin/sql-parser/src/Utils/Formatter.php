...
class Formatter
{
    ...
    {
        return array(array('type' => Token::TYPE_KEYWORD, 'flags' => Token::FLAG_KEYWORD_RESERVED, 'html' => 'class="sql-reserved"', 'cli' => "\33[35m", 'function' => 'strtoupper'), array('type' => Token::TYPE_KEYWORD, 'flags' => 0, 'html' => 'class="sql-keyword"', 'cli' => "\33[95m", 'function' => 'strtoupper'), array('type' => Token::TYPE_COMMENT, 'flags' => 0, 'html' => 'class="sql-comment"', 'cli' => "\33[37m", 'function' => ''), array('type' => Token::TYPE_BOOL, 'flags' => 0, 'html' => 'class="sql-atom"', 'cli' => "\33[36m", 'function' => 'strtoupper'), array('type' => Token::TYPE_NUMBER, 'flags' => 0, 'html' => 'class="sql-number"', 'cli' => "\33[92m", 'function' => 'strtolower'), array('type' => Token::TYPE_STRING, 'flags' => 0, 'html' => 'class="sql-string"', 'cli' => "\33[91m", 'function' => ''), array('type' => Token::TYPE_SYMBOL, 'flags' => 0, 'html' => 'class="sql-variable"', 'cli' => "\33[36m", 'function' => ''));
    }
    ...
    public function formatList($list)
    {
        ...
        for ($list->idx = 0; $list->idx < $list->count; ++$list->idx) {
            ...
            if ($curr->type === Token::TYPE_WHITESPACE) {
                ...
                if (strpos($curr->token, "\n") !== false && ($prev !== null && $prev->type === Token::TYPE_COMMENT || $next !== null && $next->type === Token::TYPE_COMMENT)) {
                    $lineEnded = true;
                }
            ...
            if ($prev !== null) {
                ...
                // Inline JOINs
                if ($prev->type === Token::TYPE_KEYWORD && isset(JoinKeyword::$JOINS[$prev->value]) || in_array($curr->value, array('ON', 'USING'), true) && isset(JoinKeyword::$JOINS[$list->tokens[$list->idx - 2]->value]) || isset($list->tokens[$list->idx - 4]) && isset(JoinKeyword::$JOINS[$list->tokens[$list->idx - 4]->value]) || isset($list->tokens[$list->idx - 6]) && isset(JoinKeyword::$JOINS[$list->tokens[$list->idx - 6]->value])) {
                    $lineEnded = false;
                }
                ...
                if ($prev->type === Token::TYPE_OPERATOR && $prev->value === ',') {
                    ...
                    if (end($blocksLineEndings) === true || empty(self::$INLINE_CLAUSES[$lastClause]) && !$shortGroup && $this->options['parts_newline']) {
                        $lineEnded = true;
                    }
                }
                // Handling brackets.
                ...
                if ($lineEnded) {
                    ...
                    if ($prev->keyword === 'DELIMITER' || !($prev->type === Token::TYPE_OPERATOR && ($prev->value === '.' || $prev->value === '(') || $curr->type === Token::TYPE_OPERATOR && ($curr->value === '.' || $curr->value === ',' || $curr->value === '(' || $curr->value === ')') || $curr->type === Token::TYPE_DELIMITER && mb_strlen($curr->value, 'UTF-8') < 2)) {
                        $ret .= ' ';
                    }
                }
            }
    ...
    {
        return str_replace(array("\0", "\1", "\2", "\3", "\4", "\5", "\6", "\7", "\10", "\t", "\n", "\v", "\f", "\r", "\16", "\17", "\20", "\21", "\22", "\23", "\24", "\25", "\26", "\27", "\30", "\31", "\32", "\33", "\34", "\35", "\36", "\37"), array('\\x00', '\\x01', '\\x02', '\\x03', '\\x04', '\\x05', '\\x06', '\\x07', '\\x08', '\\x09', '\\x0A', '\\x0B', '\\x0C', '\\x0D', '\\x0E', '\\x0F', '\\x10', '\\x11', '\\x12', '\\x13', '\\x14', '\\x15', '\\x16', '\\x17', '\\x18', '\\x19', '\\x1A', '\\x1B', '\\x1C', '\\x1D', '\\x1E', '\\x1F'), $string);
    }
    /**
    ...
    public function toString($token)
    {
        ...
        if ($this->options['type'] === 'cli') {
            if ($prev != "\33[39m") {
                $prev = "\33[39m";
                return "\33[39m" . $this->escapeConsole($text);
            }
            return $this->escapeConsole($text);
    ...
    {
        if ($token->type === Token::TYPE_KEYWORD && isset(Parser::$STATEMENT_PARSERS[$token->keyword]) || $token->type === Token::TYPE_NONE && strtoupper($token->token) === 'DELIMITER') {
            return 2;

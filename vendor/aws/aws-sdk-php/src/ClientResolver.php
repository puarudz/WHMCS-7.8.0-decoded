...
class ClientResolver
{
    ...
    public static function _apply_endpoint_provider(callable $value, array &$args)
    {
        if (!isset($args['endpoint'])) {
            ...
            $result = EndpointProvider::resolve($value, ['service' => $endpointPrefix, 'region' => $args['region'], 'scheme' => $args['scheme']]);
    ...
    public static function _apply_endpoint($value, array &$args, HandlerList $list)
    {
        ...
        if (empty($parts['scheme']) || empty($parts['host'])) {
            throw new IAE('Endpoints must be full URIs and include a scheme and host');
        }
        $args['endpoint'] = $value;
    }
    ...
    {
        return PartitionEndpointProvider::defaultProvider()->getPartition($args['region'], $args['service']);
    }

...
class StandardSessionConnection implements SessionConnectionInterface
{
    ...
    {
        $this->client = $client;
        $this->config = $config + ['table_name' => 'sessions', 'hash_key' => 'id', 'session_lifetime' => (int) ini_get('session.gc_maxlifetime'), 'consistent_read' => true, 'batch_config' => []];
    }
    public function read($id)
    {
        ...
        try {
            ...
            $result = $this->client->getItem(['TableName' => $this->config['table_name'], 'Key' => $this->formatKey($id), 'ConsistentRead' => (bool) $this->config['consistent_read']]);
            // Get the item values
    ...
    public function write($id, $data, $isChanged)
    {
        ...
        $attributes = ['expires' => ['Value' => ['N' => (string) $expires]], 'lock' => ['Action' => 'DELETE']];
        if ($isChanged) {
            if ($data != '') {
        ...
        try {
            return (bool) $this->client->updateItem(['TableName' => $this->config['table_name'], 'Key' => $this->formatKey($id), 'AttributeUpdates' => $attributes]);
        } catch (DynamoDbException $e) {
    ...
    {
        try {
            return (bool) $this->client->deleteItem(['TableName' => $this->config['table_name'], 'Key' => $this->formatKey($id)]);
        } catch (DynamoDbException $e) {

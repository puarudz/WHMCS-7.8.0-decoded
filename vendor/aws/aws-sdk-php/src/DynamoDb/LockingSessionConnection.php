...
class LockingSessionConnection extends StandardSessionConnection
{
    ...
    {
        parent::__construct($client, $config + ['max_lock_wait_time' => 10, 'min_lock_retry_microtime' => 10000, 'max_lock_retry_microtime' => 50000]);
    }
    /**
     * {@inheritdoc}
    ...
    public function read($id)
    {
        ...
        $params = ['TableName' => $this->config['table_name'], 'Key' => $this->formatKey($id), 'Expected' => ['lock' => ['Exists' => false]], 'AttributeUpdates' => ['lock' => ['Value' => ['N' => '1']]], 'ReturnValues' => 'ALL_NEW'];
        ...
        while (true) {
            try {
                ...
                if ($e->getAwsErrorCode() === 'ConditionalCheckFailedException' && time() < $timeout) {
                    usleep(rand($this->config['min_lock_retry_microtime'], $this->config['max_lock_retry_microtime']));
                } else {
                    break;
                }
            }

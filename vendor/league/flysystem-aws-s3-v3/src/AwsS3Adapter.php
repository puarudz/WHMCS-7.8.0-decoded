...
class AwsS3Adapter extends AbstractAdapter implements CanOverwriteFiles
{
    ...
    {
        $command = $this->s3Client->getCommand('copyObject', ['Bucket' => $this->bucket, 'Key' => $this->applyPathPrefix($newpath), 'CopySource' => urlencode($this->bucket . '/' . $this->applyPathPrefix($path)), 'ACL' => $this->getRawVisibility($path) === AdapterInterface::VISIBILITY_PUBLIC ? 'public-read' : 'private'] + $this->options);
        try {
            $this->s3Client->execute($command);
        } catch (S3Exception $e) {
            return false;
    ...
    {
        $options = ['Bucket' => $this->bucket, 'Key' => $this->applyPathPrefix($path)];

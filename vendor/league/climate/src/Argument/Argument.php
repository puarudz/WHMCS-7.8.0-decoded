...
class Argument
{
    ...
    {
        $allowed = ['prefix', 'longPrefix', 'description', 'required', 'noValue', 'castTo', 'defaultValue'];
    ...
    protected function setCastTo($castTo)
    {
        if (!in_array($castTo, ['string', 'int', 'float', 'bool'])) {
            throw new \Exception("An argument may only be cast to the data type " . "'string', 'int', 'float', or 'bool'.");
        }
